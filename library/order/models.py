...
class Order(models.Model):
    ...
    planed_end_at = models.DateTimeField(default=None)
    ...
    def __str__(self):
        ...
        if self.end_at == None:
            return f"\'id\': {self.pk}, " \
                   ...
                   f" \'planed_end_at\': \'{self.planed_end_at}\'"
        else:
            return f"\'id\': {self.pk}, " \
                   f"\'user\': CustomUser(id={self.user.pk})," \
                   f" \'book\': Book(id={self.book.pk})," \
    ...
    @staticmethod
    def create(user, book, planed_end_at):
        ...
        if book.id in books and book.count == 1:
            return None
        try:
            order = Order(user=user, book=book, planed_end_at=planed_end_at)
            order.save()
            return order
        except ValueError:
            return None
    ...
    def update(self, planed_end_at=None, end_at=None):
        if planed_end_at != None:
